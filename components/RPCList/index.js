...
const Row = ({ values, chain, isEthMainnet }) => {
  ...
  const isPublicNode = data?.url === `https://polygon.publicnode.com` || data?.url === `http://test.publicnode.com` || data?.url === `http://test-node.allnodes.me` || data?.url === 'http://188.166.75.111' || data?.url === 'matic-ams.allnodes.me';
